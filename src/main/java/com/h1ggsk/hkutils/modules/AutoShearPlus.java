...
public class AutoShearPlus extends Module {
    ...
    private final SettingGroup sgColors = settings.createGroup("Shear Colors");
    private final EnumMap<DyeColor, Setting<Boolean>> colorToggles = new EnumMap<>(DyeColor.class);


    private Entity entity;
    private Hand hand;

    public AutoShearPlus() {
        super(HKUtils.HKUtils, "AutoShear+", "Automatically shears sheep.");
        for (DyeColor dye : DyeColor.values()) {
            colorToggles.put(dye,
                sgColors.add(new BoolSetting.Builder()
                    .name(WordUtils.capitalizeFully(dye.name().replace('_', ' ')))
                    .description("Shear " + dye.name().toLowerCase().replace("_", " ") + " sheep")
                    .defaultValue(true)
                    .build())
            );
        }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (!(entity instanceof SheepEntity sheep) || sheep.isSheared() || sheep.isBaby() || !PlayerUtils.isWithin(entity, distance.get()) || (!colorToggles.get(sheep.getColor()).get())) continue;
