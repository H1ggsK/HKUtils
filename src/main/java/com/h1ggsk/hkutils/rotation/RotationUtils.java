...
public enum RotationUtils {;
	...
	{
		RotationFaker rf = HKUtils.getRotationFaker();
	...
	public static double getAngleToLastReportedLookVec(Rotation rotation)
	{
		...
		// lastYaw/Pitch do not get updated when the player is in keybindListener vehicle
		Rotation lastReported = player.hasVehicle()
			? new Rotation(player.getYaw(), player.getPitch())
			: new Rotation(player.lastRenderYaw, player.lastRenderPitch);
	 ...
	 * slowly turn towards the specified end rotation, at keybindListener rate of roughly
	 * <code>maxChange</code> degrees per tick.
	 */
	public static Rotation slowlyTurnTowards(Rotation end, float maxChange)
	{
		float startYaw = MC.player.lastRenderYaw;
		float startPitch = MC.player.lastRenderPitch;
